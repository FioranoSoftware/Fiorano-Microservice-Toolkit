...
/**
 * Removes the Least Recently Updated entries from the Cache
 *
 * @author Venkat
 */
public class LRUpdatedHashtableStorage extends AbstractLRUHashtableStorage {
    public LRUpdatedHashtableStorage(CachePM configuration) {
        ...
        if (configuration.getInitialCapaity() > 0) {
            this.hashmap = new LRUHashMap(configuration.getInitialCapaity(), configuration.getThresholdSize(), false);
        } else {
            this.hashmap = new LRUHashMap(configuration.getThresholdSize(), false);
        }
    }
    ...
    public CacheEntry update(CacheEntry entry) throws CacheException {
        ...
        CacheEntry oldEntry;
        synchronized (lock) {
            oldEntry = remove(entry.getKey());
            if (oldEntry != null) {
                oldEntry.getValue().putAll(entry.getValue());
                hashmap.put(oldEntry.getKey(), oldEntry.getValue());
            } else {
