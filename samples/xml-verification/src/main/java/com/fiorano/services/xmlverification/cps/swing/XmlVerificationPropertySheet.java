/*
 * Copyright (c) Fiorano Software and affiliates. All rights reserved. http://www.fiorano.com
 * The software in this package is published under the terms of the CPAL v1.0
 * license, a copy of which has been included with this distribution in the
 * LICENSE.txt file.
 */

package com.fiorano.services.xmlverification.cps.swing;

import com.fiorano.edbc.framework.service.Constants;
import com.fiorano.edbc.framework.service.configuration.IServiceConfiguration;
import com.fiorano.edbc.framework.service.cps.JMXPropertySheet;
import com.fiorano.esb.wrapper.CPSESBUtil;
import com.fiorano.xmlverification.model.XmlVerificationPM;

public class XmlVerificationPropertySheet extends JMXPropertySheet {

    @Override
    protected IServiceConfiguration getDefaultConfiguration() {
        return new XmlVerificationPM();
    }

    @Override
    protected void onClose(boolean finished, Object configuration, CPSESBUtil cpsesbUtil) {
        super.onClose(finished, configuration, cpsesbUtil);
        if (finished) {
            XmlVerificationPM config = (XmlVerificationPM) configuration;
            cpsesbUtil.getServiceInstanceAdapter().getOutputPortInstance(Constants.OUT_PORT_NAME).setSchema(config.getBody());
        }
    }
}