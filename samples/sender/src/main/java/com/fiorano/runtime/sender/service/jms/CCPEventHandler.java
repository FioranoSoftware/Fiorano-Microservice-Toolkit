...
package com.fiorano.runtime.sender.service.jms;
...
import com.fiorano.runtime.sender.service.SenderCommandLineParams;
import com.fiorano.runtime.sender.service.SenderService;
...
public class CCPEventHandler extends AbstractCCPEventHandler {
    ...
    public CCPEventHandler(SenderService service, ICCPEventGenerator ccpEventGenerator, Logger logger) {
        ...
        switch (event.getCommand()) {
            ...
            case SET_LOGLEVEL:
                for (Map.Entry<String, String> logLevelEntry : event.getArguments().entrySet()) {
                    SenderCommandLineParams senderCommandLineParams = service.getSenderParams();
                    Logger logger = LoggerUtil.getServiceLogger(logLevelEntry.getKey(),senderCommandLineParams.getConnectionFactory(),
                            senderCommandLineParams.getServiceGUID());
    ...
    protected void handleLogLevelEvent(DataRequestEvent event, DataEvent dataEvent) {
        ...
        if (event instanceof LogLevelRequestEvent) {
            for (String loggerName : ((LogLevelRequestEvent) event).getLoggerNames()) {
                SenderCommandLineParams senderCommandLineParams = service.getSenderParams();
                Logger logger = LoggerUtil.getServiceLogger(loggerName, senderCommandLineParams.getConnectionFactory(),
                        senderCommandLineParams.getServiceGUID());
