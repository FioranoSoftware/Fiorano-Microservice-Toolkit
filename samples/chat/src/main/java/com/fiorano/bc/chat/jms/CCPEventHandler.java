...
public class CCPEventHandler extends AbstractCCPEventHandler {
    ...
    public CCPEventHandler(ChatService service, ICCPEventGenerator ccpEventGenerator, Logger logger) {
        ...
        switch (event.getCommand()) {
            ...
            case SET_LOGLEVEL:
                for (Map.Entry<String, String> logLevelEntry : event.getArguments().entrySet()) {
                    CommandLineParameters commandLineParameters = service.getCmdLineParameters();
                    Logger logger = LoggerUtil.getServiceLogger(logLevelEntry.getKey(),commandLineParameters.getConnectionFactory(),
                            commandLineParameters.getServiceGUID());
    ...
    protected void handleLogLevelEvent(DataRequestEvent event, DataEvent dataEvent) {
        ...
        if (event instanceof LogLevelRequestEvent) {
            for (String loggerName : ((LogLevelRequestEvent) event).getLoggerNames()) {
                CommandLineParameters commandLineParameters = service.getCmdLineParameters();
                Logger logger = LoggerUtil.getServiceLogger(loggerName, commandLineParameters.getConnectionFactory(),
                        commandLineParameters.getServiceGUID());
